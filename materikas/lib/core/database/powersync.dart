      ...
      try {
        ...
        for (var op in transaction.crud) {
          ...
          if (op.op == UpdateType.put) {
            ...
            data['id'] = op.id;
            debugPrint('- INSERT -');
            debugPrint('id Insert: ${op.id}-');
            debugPrint('data: ${op.opData}-');
            await table.upsert(data);
            debugPrint('INSERT BERHASIL');
          } else if (op.op == UpdateType.patch) {
            debugPrint('- UPDATE -');
            debugPrint('id Update: ${op.id}');
            debugPrint('data: ${op.opData}');
            try {
...
Future<void> openDatabase() async {
  ...
  Supabase.instance.client.auth.onAuthStateChange.listen((data) async {
    ...
    debugPrint('powersync Listened: $event');
    if (event == AuthChangeEvent.signedIn) {
      ...
      debugPrint('powersync: ${currentConnector!.db.currentStatus}');
    } else if (event == AuthChangeEvent.signedOut) {
      // Implicit sign out - disconnect, but don't delete data
      currentConnector = null;
      await db.disconnect();
      debugPrint('-LOGOUT-');
      debugPrint('powersync: logout');
      debugPrint('current user: ${Supabase.instance.client.auth.currentUser}');
      debugPrint('db ${db.currentStatus}');
